...
final class TroubleshootTests: XCTestCase {
    ...
    func testConfigLoginsRestarts() throws {
        ...
        // Round 13: per device and source — web01's four and the one on each router, each a
        // Problem because 198.51.100.7 failed six times on three devices within five minutes.
        let logins = only(r, "login.failures")
        XCTAssertEqual(Set(logins.compactMap(\.device)), ["web01", "CORE-RTR1", "S5720-CORE"], logins.map(\.title).description)
        let login = try XCTUnwrap(logins.first { $0.device == "web01" })
        XCTAssertEqual(login.severity, .bad)
        XCTAssertEqual(login.category, .security)
        XCTAssertEqual(login.client, "198.51.100.7")
        XCTAssertEqual(login.count, 4)
        XCTAssertTrue(login.detail.contains("6 failures across 3 devices"), login.detail)
        XCTAssertTrue(logins.allSatisfy { $0.severity == .bad && $0.client == "198.51.100.7" })
    ...
    func testTCPProblems() {
        ...
        XCTAssertEqual(only(r, "tcp.refused").first { $0.title.contains("10.1.30.9:8443") }?.severity, .info, "one refusal: a note (round 13)")
        ...
        // The demo's lossy download has 14 data segments: under `retransMinSegments` its share
        // means little (round 13); the Flows pane still shows the loss.
        XCTAssertFalse(only(r, "tcp.retransmissions").contains { $0.title.contains("10.1.30.8:443") })
